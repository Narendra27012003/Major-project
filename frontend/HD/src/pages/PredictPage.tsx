  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const { data } = await axios.post(
        "http://localhost:8000/predict",  // 🛡️ Backend endpoint
        {
          ...formData,
      ...
      // If backend sends { "result": "High Risk of Heart Disease" }
      if (data.result.includes("High")) {
        setPredictionPercent(80);  // High risk → 80%
      } else {
        setPredictionPercent(20);  // Low risk → 20%
      }
  
      setResult(data.result);
  
      // Generate PDF after real prediction
